...
resume_message = """
For each resume provided {Resume} which were retrieved based on the job description : {job_description}, provided by user,
...
QnA_msg = """ 
You are a helpful and professional AI assistant named Lumea designed to assist users with resume retrieval and job matching. Your primary tasks include:
...
def get_resume(job_description: str) -> dict:
    ...
    """
    model = Model()
    Document = model.search_from_database(job_description)
    print(Document)
...
class Agent:
        ...
        if os.path.exists("testing_data.json"):
            with open("testing_data.json", 'r') as file:
                try:
                    ...
                    print("Warning: JSON file is empty or invalid. Using empty dict.")
                    self.data = []
        else:
            self.data = []
    ...
    def llm_summary(self, state: Agentstate):
        ...
        resume = []
        doc = [item for sublist in state["resumes"] for inner in sublist for item in inner]
        for text in doc:
            text = text.strip()
            ...
            # Step 3: Remove all punctuation except , and .
            allowed = {',', '.'}
            cleaned = ''.join(ch for ch in text if ch.isalnum() or ch.isspace() or ch in allowed)

            # Step 4: Remove space before punctuation, ensure space after comma/period if needed
            cleaned = re.sub(r'\s([.,])', r'\1', cleaned)
            cleaned = re.sub(r'([.,])(?=[^\s])', r'\1 ', cleaned)
            resume.append(cleaned.strip())
            
        resume_prompt = PromptTemplate.from_template(resume_message)
        fomratted_resume_prompt = resume_prompt.format(Resume=resume,job_description=state["user_input"][-1])
        output = self.model.invoke(str(fomratted_resume_prompt))
        print(state)
        new_entry = {
            "job_description": str(state["user_input"][-1]),
            "input_query": str(fomratted_resume_prompt),
            "documents": resume,
